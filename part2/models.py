...
class Card(models.Model):
    id = EncryptedCharField(models.AutoField(primary_key=True))
    data = EncryptedCharField(models.BinaryField(unique=True))
    product = models.ForeignKey('LegacySite.Product', on_delete=models.CASCADE, default=None)
    amount = EncryptedIntegerField(models.IntegerField())
    fp = EncryptedCharField(models.CharField(max_length=100, unique=True))
    user = models.ForeignKey('LegacySite.User', on_delete=models.CASCADE)
    used = EncryptedTextField(models.BooleanField(default=False))
